  ...
  return (
    <div className='flex justify-center items-center relative w-full py-7 max-sm:p-3'>
        ...
        {itemsList.map((item: ProductItem) => (
          <Link href={`/${category.toLowerCase()}/${item.name.toLowerCase().replace(/\s+/g, '-')}`} key={item.name} className='max-xl:col-span-1 flex flex-col items-center xl:w-28 sm:w-36 w-28 h-96 gap-y-5 xl:hover:w-32 transition-all duration-200 cursor-pointer group'>
